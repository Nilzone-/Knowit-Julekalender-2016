...
let lower = [], higher = [];
const map = ['0', 'I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X', 'XI', 'XII', 'XIII'];
const message = 'Your message was received with gratitude! We do not know about you, but Christmas is definitely our favourite holiday. The tree, the lights, all the presents to unwrap. Could there be anything more magical than that?! We wish you a happy holiday and a happy new year!'
  .replace(/[\.\,\?\!\s]+/g, '')
  .split('')
  .map(str => {
    let number = str.toLowerCase().charCodeAt(0) - 96;
    let n = Math.round(number / 2);
    if(number % 2 !== 0 ) { lower.push(map[n]); higher.unshift(map[n-1]); }
    else { lower.push(map[n]); higher.unshift(map[n]); }
  });

console.log(lower.concat(higher).join(','));